...
We use attention-weight-based eviction (following H\textsubscript{2}O~\citep{h2o2023}), retaining tokens with the highest cumulative attention scores regardless of position. This is critical: position-based strategies like StreamingLLM~\citep{streamingllm2024} perform well on perplexity (which tests local prediction) but catastrophically fail on retrieval tasks (NIAH) because the target information may reside at any position. Attention-aware eviction naturally preserves high-information tokens wherever they occur.

For each generation step, we maintain a running attention score $S_j = \sum_{t} a_{t,j}$ for each cached token $j$, and retain the top-$(1 - f_{\text{evict}})$ fraction by cumulative score:
\begin{equation}
    \mathcal{S} = \{j : S_j \geq \text{quantile}_{f_{\text{evict}}}(S)\}
\end{equation}
...
\paragraph{Why not position-based eviction?}
StreamingLLM (retain first 128 + recent window, evict middle) achieves excellent PPL ($<$1\% at 50\% eviction) because middle tokens contribute little to \emph{local} next-token prediction. However, it achieves only 20\% NIAH accuracy at 85\% eviction because needles at arbitrary positions are discarded. Attention-aware eviction resolves this: high-information tokens receive high attention scores and are retained regardless of position.
