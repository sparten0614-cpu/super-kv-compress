...
\begin{itemize}
    \item \textbf{Selective recompute as the sole path beyond the eviction limit:} Our experiments with four scoring strategies---including Expected Attention (K-statistics-based)---confirm that no eviction scoring approach can overcome the causal limitation at high compression rates. The practical alternative is selective recompute: store all tokens at ultra-low precision plus token IDs; at decode time, use low-precision attention to identify top-$k$ important tokens and recompute their full-precision KV on the fly. Our theoretical analysis suggests 34.6$\times$ compression at 100\% NIAH with $\sim$11\% compute overhead (\S\ref{sec:theory}).
