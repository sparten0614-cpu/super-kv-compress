...
\begin{remark}[V is more sensitive than K]
The $E_V$ term scales with attention concentration $\kappa$, while $E_K$ scales with $1/T^2$. For sharp attention patterns ($\kappa$ large), $E_V$ dominates, explaining our empirical finding that V$=$2-bit causes catastrophic failure while K$=$6-bit is safe. This contradicts the KVTuner conclusion that Keys are universally more sensitive---the sensitivity ranking depends on the attention distribution's sharpness.
\end{remark}
...
The four-tier thresholds $\tau_1 < \tau_2 < \tau_3$ (Table~\ref{tab:tiers}) are derived from the quantization noise floor $\sigma_{\text{floor}}$:
